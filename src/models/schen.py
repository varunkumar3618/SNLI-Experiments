...
class SChen(SNLIModel):
    def __init__(self,
                 hidden_size, use_peepholes,
                 l2_reg, use_lens=True, use_dropout=True,
                 *args, **kwargs):
        super(SChen, self).__init__(use_lens=use_lens, use_dropout=use_dropout,
                                    *args, **kwargs)
    ...
    def add_prediction_op(self):
        with tf.variable_scope("prediction"):
            ...
            prem_hiddens, _, hyp_hiddens, _ = self.encoding(prem_embed, hyp_embed, "encoding1")
            prem_tilda, hyp_tilda = self.attention(prem_hiddens, hyp_hiddens, "attention1")
            prem_m, hyp_m = self.collection(prem_hiddens, prem_tilda, hyp_hiddens, hyp_tilda)

            prem_hiddens, _, hyp_hiddens, _ = self.encoding(prem_m, hyp_m, "encoding2")
            prem_tilda, hyp_tilda = self.attention(prem_hiddens, hyp_hiddens, "attention2")
            prem_m, hyp_m = self.collection(prem_hiddens, prem_tilda, hyp_hiddens, hyp_tilda)
